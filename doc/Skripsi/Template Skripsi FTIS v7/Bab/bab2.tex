...
\begin{algorithmic}[1]
	...
	\IF{attribute\_list is empty}
		\RETURN  N as leaf node labeled with the majority class in D; \COMMENT {majority voting}
	\ENDIF
	...
	\IF{splitting\_attribute is discrete valued and multiway splits allowed}
		\STATE attribute\_list $\leftarrow$ attribute\_list - splitting\_attribute; \COMMENT{remove splitting\_attribute}
	\ENDIF
	\FORALL{outcome j of splitting\_criterion}
		\STATE let D\lowercase{j} be the set of data tuples in D satisfying outcome j; \COMMENT{a partition}
		\IF{D\lowercase{j} is empty}
...
%for each outcome j of splitting\_criterion // partition the tuples and from subtrees for each partition

%let D\lowercase{j} be the set of data tuples in D satisfying outcome j; //a partition
